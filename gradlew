...
if ! "$cygwin" && ! "$darwin" && ! "$nonstop" ; then
    case $MAX_FD in  #(
      max*)
        ...
        # See https://bugs.openjdk.java.net/browse/JDK-8146397 for details.
        MAX_FD=$( ulimit -H -n ) ||
            warn "Could not query maximum file descriptor limit"
    esac
    case $MAX_FD in  #(
      '' | soft | *':*' | *[!0-9]*)
        # The max file descriptor limit for the process.
        MAX_FD=$( ulimit -n ) || warn "Could not query maximum file descriptor limit"
        # The max file descriptor is incorrect, so continue without it.
    esac
fi
